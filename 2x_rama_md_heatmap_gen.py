...
def main( ):
  ...
  args = UserInput()
  if args.residue is None:    # amino acid for reference AA density map
    args.residue  = 'Gen'
  if args.interval is None:   # default degree interval
    args.interval = 2
  else: args.interval = float(args.interval)
  if args.fraction is None:   # default fraction for histogram data cutoff
    args.fraction = 33
  else: args.fraction = float(args.fraction)
  if args.smoothen is None:
    args.smoothen = 1.15
  else: args.smoothen = float(args.smoothen)
  if args.t_step is None:     # colorbar tick spacing per histogram digits unit
    args.t_step   = 4
  else: args.t_step = float(args.t_step)
  if args.c_step is None:     # contour spacing per histogram digits unit 
    args.c_step   = 4
  else: args.c_step = float(args.c_step)
  ...
  res_obj = InputRamaData( args.in_file,  args.interval, args.fraction,
...
def UserInput():
  ...
  p.add_argument('-fraction', dest='fraction', required=False,
                 help='Cutoff fraction of the maximum Histogram value (def: 33)')
  p.add_argument('-t_step', dest='t_step', required=False,
                 help='Colorbar tick spacing per Histogram digits value (def: 4)')
  p.add_argument('-c_step', dest='c_step', required=False,
