...
#  v.3  20.12.30  update readin column selection
#
#  Read in a file with X-/Y-axis data together, and
...
#####  
#
#  e.g.> x.py -in     X-Y_file.txt    <- X-/Y-axis are together
#             -col    1 2  -sep ','
...
def main():
  ...
  args.cols = np.array(list(map(int, args.cols))) - 1
...
######################
  ...
  raw = pd.read_csv(args.in_file, sep=args.sep, comment='#')
  df  = raw[args.cols]
  df.columns = ['x','y']
...
def cmdlineparse():
  ...
  p.add_argument('-in', dest='in_file', required=True,
                 help='X/Y-axis time-series file of one residue (gzip okay)')
  ...
  p.add_argument("-sep", dest='sep', required=False, default='\s+',
                 help="Delimiter (def: ' ')", metavar='<sep>')
  p.add_argument("-col", dest="cols", required=False, nargs='+', default=[1,2],
                 help="Columns as X/Y-axis (def: 1 2)", metavar="<col_x col_y>")
